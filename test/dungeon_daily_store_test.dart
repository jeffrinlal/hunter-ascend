...
void main() {
  ...
  test('saved dungeon round-trips and is reused on re-entry', () async {
    ...
    expect(
      reloaded.map((o) => o.title).toList(),
      generated.monsters.map((o) => o.title).toList(),
    );
    expect(
      reloaded.map((o) => o.type).toList(),
      generated.monsters.map((o) => o.type).toList(),
    );
    expect(
      reloaded.map((o) => o.target).toList(),
      generated.monsters.map((o) => o.target).toList(),
    );
    expect(
      reloaded.map((o) => o.monster).toList(),
      generated.monsters.map((o) => o.monster).toList(),
      reason:
          'AI-supplied monster names are structured data and '
          'must round-trip',
    );
    expect(
      reloaded.every((o) => o.startValue == null),
      isTrue,
      reason:
          'freshly generated rows carry no snapshot until the '
  ...
  test('snapshot progress persists across re-entries (leaving / reopening / '
      'app restart)', () async {
    SharedPreferences.setMockInitialValues({});
    ...
    await DungeonDailyStore.saveProgress('E', [
      ...generated.monsters,
      generated.boss,
    ]);
  ...
  test(
    'snapshot progress is monotonic — it never resets or moves backwards',
    () async {
  ...
  test(
    'saveProgress merges into the stored entry without clobbering it',
    () async {
      SharedPreferences.setMockInitialValues({});
      ...
      expect(
        state.objectivesForToday()!.length,
        3,
        reason: 'progress writes must not touch the dungeon itself',
      );
      expect(
        state.bossForToday()!.progress,
        0,
      ...
      expect(
        (await DungeonDailyStore.load('E')).objectivesForToday()![1].startValue,
        300,
        reason: 'first capture wins — snapshots are never re-anchored',
      );
    },
  );
  ...
  test(
    'legacy entries migrate: walk/run → steps, manual types dropped',
    () async {
      ...
      expect(
        monsters.every((o) => o.monster == null),
        isTrue,
        reason:
            'legacy rows carry no monster — the fallback bestiary '
            'supplies the identity',
      );
      expect(
        monsters.every((o) => o.startValue == null),
        isTrue,
        reason:
            'legacy rows carry no snapshot — the session captures '
      ...
      expect(
        ...
        isNull,
        reason:
            'legacy entries have no boss row — the session manager '
            'substitutes the fallback boss',
      );
    },
  );

  test('clear + reward claim state persists exactly once per day', () async {
    SharedPreferences.setMockInitialValues({});

    await DungeonDailyStore.saveObjectives('E', sampleDungeon());
    var state = await DungeonDailyStore.load('E');
    expect(state.completedToday, isFalse);
    expect(state.completedAtDate, isNull);
    expect(state.rewardClaimed, isFalse);

    // Clearing stamps the completion DATE and TIME.
    await DungeonDailyStore.markCleared('E');
    state = await DungeonDailyStore.load('E');
    expect(state.completedToday, isTrue);
    expect(
      state.completedAtDate,
      isNotNull,
      reason: 'the completion time must be persisted',
    );
    expect(
      state.rewardClaimed,
      isFalse,
      reason: 'clearing alone does not claim the reward',
    );

    // Claiming flips the exactly-once gate…
    await DungeonDailyStore.markRewardClaimed('E', claimed: true);
    state = await DungeonDailyStore.load('E');
    expect(state.rewardClaimed, isTrue);
    expect(
      state.completedToday,
      isTrue,
      reason: 'claiming must not touch the clear state',
    );
    expect(
      state.objectivesForToday(),
      isNotNull,
      reason:
          'a cleared dungeon is never regenerated — the stored '
          'objectives stay intact',
    );

    // …and the rollback path (failed XP award) can unset it again.
    await DungeonDailyStore.markRewardClaimed('E', claimed: false);
    expect((await DungeonDailyStore.load('E')).rewardClaimed, isFalse);

    // A second clear the same day keeps the ORIGINAL completion time.
    final firstCompletedAt = state.completedAtDate;
    await DungeonDailyStore.markCleared('E');
    expect(
      (await DungeonDailyStore.load('E')).completedAtDate,
      firstCompletedAt,
    );
  });

  test(
    'reward claim flag from a previous day does not leak into today',
    () async {
      final yesterday = DateTime.now()
          .subtract(const Duration(days: 1))
          .toString()
      ...
      expect(
        state.completedToday,
        isFalse,
        reason: 'yesterday\'s clear must not count today',
      );
      expect(
        state.rewardClaimed,
        isTrue,
        reason:
            'the raw flag stays in the entry (the date guard makes '
            'it inert) — the session manager only applies it when '
            'completedToday',
      );
    },
  );
  ...
  test(
    'concurrent writes are serialized and never clobber objectives',
    () async {
      SharedPreferences.setMockInitialValues({});
